...
= News
* forked version 0.0.4.3 to add some new features and support for ChiliProject 3.x
* made the layout more compact and better to read especially for many tasks
* added drag and drop support to sort tasks (still a bit buggy - working on it)

== Known problems
Currently not all features are available for ChiliProject users!

This Redmine plugin is not compatible to the ChiliProject activity list any more - enabling the activity display of tasks will lead to internal errors / crash of ChiliProject!

When sorting items by dragging and dropping sometimes the edit input field for the task text opens on mour button release. This issue can be work around by moving the mouse away from the task title while dragging... I'll try to fix that as soon as possible.
...
== Release 0.0.5.0
* improved compatibility to ChiliProject 3.x (tested with 3.7)
* added more drag and drop features
* added automatic/floating multiple column layout
* improved display of tasks

== Release 0.0.4.3 -> FORKED VERSION
